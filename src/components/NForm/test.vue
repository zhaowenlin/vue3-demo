  ...
  methods: {
    ...
    getFormListByNumber(h) {
      ...
      for (let i = 0; i < this.formList.length; i += grid) {
        ...
        for (let j = 0; j < grid && i + j < this.formList.length; j++) {
          const children = this.formList[i + j]
          if (!children) break
          const childrenItem = this.getFormItem(h, children, this.getContent(h, children))
          const childrenParts = h(getPrefix('col', this.lib), {
            props: {
              span: 24 / grid
            }
          }, [
        ...
        const row = this.getRow(h, childrenList)
        list.push(row)
      }
      return list
    },
    ...
    getFormListByArray(h) {
      ...
      for (let i = 0; i < this.formList.length;) {
        const childrenList = []
        const grid = this.grid[gridIndex]
